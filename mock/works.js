...
const putFakeWork = (req, res) => {
  ...
  const { uuid, type, title, uri } = req.body;
  res.json({
    status: 'ok',
    count: 1,
    code: 200,
    data: [{ title, uri, type, uuid }]
  });
};
...
const getSingleWork = uuid => {
  return works.filter(item => item.UUID === uuid);
};
