...
class PublicationList extends Component {
  ...
  state = {
    ...
    pagination: {
      position: 'both',
      pageSize: parseInt(this.pageSize, 10) || 10,
  ...
  handleChange = (pageOpts, filters, sorter) => {
    localStorage.setItem('metrics-borker-works-pagesize', pageOpts.pageSize);
    this.setState({
      pagination: pageOpts,
  ...
  render() {
    ...
    const types = [...new Set(publication.map(item => item.type))];
    const availableFilters = types.map(type => ({ text: type, value: type }));
    ...
    const onTitleSearch = (value, record) =>
      record.title
        .toString()
        .toLowerCase()
    ...
    const onUriSearch = (value, record) =>
      record.URI.map(uri => uri.URI.toLowerCase().replace(/-/g, ''))
        .toString()
    ...
    /* it's ok to use indeces as keys in title/uri search; always same order */
    const columns = [
      {
        title: 'Title',
        dataIndex: 'title',
        ...
        filterIcon: filtered => (
          <Icon
            id="title-search-icon"
            type="search"
            style={{ color: filtered ? '#108ee9' : '#aaa' }}
          />
        ),
        onFilter: onTitleSearch,
        onFilterDropdownVisibleChange: onFilterChange,
        render: (text, record) => {
    ...
    const description = (
      <Fragment>
        <span>You may click </span> [<Icon type="search" />] (on column headers)
        to search by publication title and/or URI. You may also click on [
        <Icon type="filter" />] to filter by work type.
      </Fragment>
    );

    return (
      <PageHeaderLayout title="Publications" content={description}>
        <Fragment>
          <Card
