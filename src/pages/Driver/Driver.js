...
import { Button, Row, Col, Icon, Card, Tooltip } from 'antd';
...
export default class Analysis extends Component {
  ...
  render() {
    ...
    const actionButton = (
      <Button icon="play-circle-o">Run</Button>
    );

    const actionButtonDisabled = (
      <Button icon="play-circle-o" disabled>Run</Button>
    );
    ...
    const drivers = driver.map(d => (
      <Col {...topColResponsiveProps} key={d.UUID}>
        <DriverCard
          ...
          total={d.name}
          loading={loading}
          avatar={<img src={d.icon || genericDriverIcon} style={{ width: 56, height: 56 }} />}
