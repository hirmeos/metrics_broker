...
describe('Work Add Form', () => {
  ...
  it('should add title', async () => {
    ...
    await page.waitForSelector('#title-NEW_TEMP_ID_0');
    await page.type('#title-NEW_TEMP_ID_0', 'New Book Title');
    await page.click('#save-title-btn-NEW_TEMP_ID_0');
    await page.waitForSelector('#edit-title-btn-NEW_TEMP_ID_0');
  ...
  it('should add uri', async () => {
    ...
    await page.waitForSelector('#uri-NEW_TEMP_ID_0');
    await page.type('#uri-NEW_TEMP_ID_0', 'info:doi:10.11647/obp.0001');
    await page.click('#save-uri-btn-NEW_TEMP_ID_0');
    await page.waitForSelector('#edit-uri-btn-NEW_TEMP_ID_0');
    const text = await page.evaluate(() => document.body.innerHTML);
  ...
  it('should add canonical uri', async () => {
    ...
    await page.waitForSelector('#uri-NEW_TEMP_ID_0');
    await page.type('#uri-NEW_TEMP_ID_0', 'info:doi:10.11647/obp.0001');
    await page.click('#uri-canonical-NEW_TEMP_ID_0');
    await page.click('#save-uri-btn-NEW_TEMP_ID_0');
    await page.waitForSelector('#edit-uri-btn-NEW_TEMP_ID_0');
    const text = await page.evaluate(() => document.body.innerHTML);
    expect(text).toContain('info:doi:10.11647/obp.0001');
  ...
  it('should add multiple titles', async () => {
    ...
    await page.click('#new-title-btn');
    await page.waitForSelector('#title-NEW_TEMP_ID_0');
    await page.type('#title-NEW_TEMP_ID_0', 'New book title 1');
    await page.click('#save-title-btn-NEW_TEMP_ID_0');
    await page.waitForSelector('#edit-title-btn-NEW_TEMP_ID_0');
    await page.click('#new-title-btn');
    await page.waitForSelector('#title-NEW_TEMP_ID_1');
    await page.type('#title-NEW_TEMP_ID_1', 'New book title 2');
    await page.click('#save-title-btn-NEW_TEMP_ID_1');
    await page.waitForSelector('#edit-title-btn-NEW_TEMP_ID_1');
    const text = await page.evaluate(() => document.body.innerHTML);
  ...
  it('should add multiple uris', async () => {
    ...
    await page.waitForSelector('#edit-uri-btn-NEW_TEMP_ID_0');
    await page.click('#new-uri-btn');
    await page.waitForSelector('#uri-NEW_TEMP_ID_1');
    await page.type('#uri-NEW_TEMP_ID_1', 'info:doi:10.11647/obp.0002');
    await page.click('#uri-canonical-NEW_TEMP_ID_1');
    await page.click('#save-uri-btn-NEW_TEMP_ID_1');
    await page.waitForSelector('#edit-uri-btn-NEW_TEMP_ID_1');
    const text = await page.evaluate(() => document.body.innerHTML);
    expect(text).toContain('info:doi:10.11647/obp.0001');
    expect(text).toContain('info:doi:10.11647/obp.0001');
